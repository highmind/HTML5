...
window.addEventListener("load",function(){
     ...
     * 渲染模式
     */
    var stage=new annie.Stage("annieEngine",750,1206,30,annie.StageScaleMode.FIXED_HEIGHT,0);
    stage.addEventListener(annie.Event.INIT_TO_STAGE,function (e) {
        Flash2x.loadScene("shiZuo",function(per){
            //加载进度
            trace("加载进度:"+per+"%");
        },function(result){
            ...
            if(result.sceneId==result.sceneTotal) {
                stage.addChild(new shiZuo.ShiZuo());
            }
        });
    })
});
